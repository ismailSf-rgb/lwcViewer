    ...
    var e = (() => {
        ...
        return o => {
            const l = o.regex,
                ...
                p = [o.APOS_STRING_MODE, o.QUOTE_STRING_MODE, h, N, _, f, {
                    match: /\$\d+/
                }, A];
            y.contains = p.concat({
                begin: /\{/,
            ...
            return {
                ...
                }, o.APOS_STRING_MODE, o.QUOTE_STRING_MODE, h, N, _, f, v, {
                    match: /\$\d+/
                }, A, k, {
                    className: "attr",
                    begin: b + l.lookahead(":"),
...
//let sfHost = 'psafinance--uat.sandbox.my.salesforce.com';
//let sfHost = 'psafinance-es--uat.sandbox.my.salesforce.com'
let sfHost = '';
chrome.tabs.query({
    active: true,
    currentWindow: true
    ...
    sfConn.getSession(sfHost).then(
        (host) => {
            ...
            // Make a request to the Salesforce API to retrieve a list of all accounts.
            //const accounts = await sfConn.rest("/services/data/v52.0/query?q=SELECT+Name+FROM+Account");
            sfConn.rest("/services/data/v52.0/tooling/query?q=SELECT+Source+,+FilePath+,+Format+FROM+LightningComponentResource").then(
                (lwcs) => {
                    // Iterate over the results and print the account names.
                    console.log('lwc..... : ', lwcs);
                    const folders = [];
                    let mapFolderFile = new Map();
                    let mapFileSource = new Map();
                    for (const lwc of lwcs.records) {
                        console.log("lwc.Format");
                        console.log(lwc.Format);
                        console.log("lwc.FilePath");
                        console.log(lwc.FilePath);
                        let filePaths = lwc.FilePath.split('/');

                        let keyToCheck = filePaths[1];
                        let valueToAdd = filePaths[2];
                        mapFileSource.set(valueToAdd, lwc.Source)
                        if (mapFolderFile.has(keyToCheck)) {
                            // Key exists, push the value into the existing array
                            mapFolderFile.get(keyToCheck).push(valueToAdd);
                        } else {
                            // Key doesn't exist, create a new array and add it to the map
                            mapFolderFile.set(keyToCheck, [valueToAdd]);
                        }
                    }

                    for (const [key, value] of mapFolderFile) {
                        folders.push({
                            "name": key,
                            "files": value
                        });
                    ...
                    function displayFileInMain(event) {
                        console.log("file selected")
                        const fileLink = event.target;
                        const div = document.querySelector("#my-div");
                        console.log("div selected: ", div);
                        div.textContent = fileLink.Source;
                    ...
                    folders.forEach(folder => {
                        ...
                        folder.files.forEach(file => {
                            ...
                            fileLink.Source = mapFileSource.get(file);
                            fileLink.addEventListener('click', displayFileInMain);
                    ...
                    // Add click event listener to the sidebar using event delegation
                    sidebar.addEventListener('click', (event) => {
                        console.log('clicked');
                        const target = event.target;
                        console.log("target.classList.contains('folder')", target.classList.contains('folder'));
                        console.log("target.tagName === 'LI'", target.tagName === 'LI');
                        console.log("target.tagName === ", target.tagName);
                        if (target.classList.contains('folder')) {
                            const files = target.querySelector('.files');
                            console.log('files', files);
                            console.log('files.style.display', files.style.display);
                            files.style.display = files.style.display === 'none' || files.style.display === '' ? 'block' : 'none';
                        } else if (target.tagName === 'A') {
                            console.log("A is clicke");
                            const allLiElements = sidebar.querySelectorAll('aside a');
                    ...
                    /*const div = document.querySelector("#my-div");
                    div.textContent = lwcs.records[2].Source;
                    hljs.highlightElement(div);*/
